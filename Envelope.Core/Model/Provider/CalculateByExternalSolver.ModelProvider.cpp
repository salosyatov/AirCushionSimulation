...
namespace EnvelopeCore {
		...
		void *user_data)
	{
		...
		TVector ** flex = NULL;
		TVector nullVector;
		if (calculationParameters.GetCalcScheme()[FORCE_FL_INDEX].ignore == false && calculationParameters.GetCalcScheme()[FORCE_FL_INDEX].sch == IMPLICIT)
		{
	...
	int  ModelProvider :: CalculateByExternalSolver(double t, double & t_out, double & dt, SkirtData * skirtData)
	{
		int N_phi = calculationParameters.GetNPhi();
		int N_zeta = calculationParameters.GetNZeta();
		...
		//13. Get optional outputs. Print remaining counters and free memory. 
